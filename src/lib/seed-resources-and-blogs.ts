/**
 * Usage: npx tsx src/lib/seed-resources-and-blogs.ts
 * Description: Checks database readiness and seeds blogs, detailed department contents, and health resources via Supabase client.
 */
...
async function seed() {
    console.log("=== Clinq Data Seeding (Blogs, Departments, Resources) ===");

    // 1. Check schema readiness
    console.log("Checking if blog_posts has image_url and content columns...");
    const { error: blogColError } = await supabase.from("blog_posts").select("image_url, content").limit(1);
    if (blogColError) {
        console.error("❌ Blog posts table is not fully migrated. Please run public.blog_posts DDL SQL first. Error:", blogColError.message);
        process.exit(1);
    }
    console.log("✅ Blog posts columns verified.");

    console.log("Checking if departments has detailed_content column...");
    const { error: deptColError } = await supabase.from("departments").select("detailed_content").limit(1);
    if (deptColError) {
        console.error("❌ Departments table is not fully migrated. Please run public.departments DDL SQL first. Error:", deptColError.message);
        process.exit(1);
    }
    console.log("✅ Departments columns verified.");

    console.log("Checking if resources table exists...");
    const { error: resTableError } = await supabase.from("resources").select("*").limit(1);
    if (resTableError) {
        console.error("❌ Resources table does not exist. Please run CREATE TABLE public.resources DDL SQL first. Error:", resTableError.message);
        process.exit(1);
    }
    console.log("✅ Resources table verified.");

    // 2. Seed departments (detailed content updates)
    ...
    console.log("\nUpdating departments detailed content...");
    for (const d of departmentsToUpdate) {
        const { error } = await supabase
            .from("departments")
            .update({ detailed_content: d.detailed_content })
            .eq("slug", d.slug);
        if (error) {
            console.error(`Failed to update department ${d.slug}:`, error.message);
        } else {
            console.log(`Updated department detail: ${d.slug}`);
        }
    }

    // 3. Seed/Update Blog Posts
    const blogPosts = [
        {
            category: 'GUIDE',
    ...
    for (const blog of blogPosts) {
        ...
        if (error) {
            console.error(`Failed to upsert blog ${blog.slug}:`, error.message);
        } else {
            console.log(`Upserted blog post: ${blog.slug}`);
        }
    }
    ...
    console.log("\nUpserting health resources...");
    for (const res of resources) {
        const { error } = await supabase
            .from("resources")
            .upsert([res], { onConflict: "slug" });
        if (error) {
            console.error(`Failed to upsert resource ${res.slug}:`, error.message);
        } else {
            console.log(`Upserted resource: ${res.slug}`);
        }
    }
