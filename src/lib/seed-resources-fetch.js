...
console.log("Got Supabase Key. Running fetch-based seeding...");
...
async function patchDepartment(slug, detailedContent) {
    const url = `${supabaseUrl}/rest/v1/departments?slug=eq.${slug}`;
    const res = await fetch(url, {
        method: 'PATCH',
        headers: headers,
        body: JSON.stringify({ detailed_content: detailedContent })
    });
    if (!res.ok) {
        throw new Error(`Failed to patch department ${slug}: ${res.status} ${await res.text()}`);
    }
    console.log(`✅ Patched department: ${slug}`);
}

async function upsertBlogPost(blog) {
    ...
    if (!res.ok) {
        throw new Error(`Failed to upsert blog ${blog.slug}: ${res.status} ${await res.text()}`);
    }
    console.log(`✅ Upserted blog post: ${blog.slug}`);
}

async function upsertResource(resource) {
    const url = `${supabaseUrl}/rest/v1/resources`;
    const res = await fetch(url, {
        method: 'POST',
        headers: {
            ...headers,
    ...
    if (!res.ok) {
        throw new Error(`Failed to upsert resource ${resource.slug}: ${res.status} ${await res.text()}`);
    }
    console.log(`✅ Upserted resource: ${resource.slug}`);
}

async function main() {
    // 2. Seeding Departments detailed_content
    const departments = [
    ...
    for (const d of departments) {
        await patchDepartment(d.slug, d.detailed_content);
    }
    ...
    console.log("🎉 Fetch-based seeding completed successfully!");
}
