    ...
    return (
      <div className="w-full h-screen flex items-center justify-center bg-[#F8F9FF]">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-sm font-[600] text-[#00355F]">Loading prescriptions...</span>
        </div>
      </div>
    );
  }
    ...
    <div className="w-full flex flex-col gap-10 px-6 py-6 bg-[#F8F9FF] font-[Manrope,sans-serif] text-[#42474F]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 w-full">
        ...
        <button
          ...
          className="flex items-center gap-4 px-8 py-3 bg-[#00355F] rounded-[12px] shadow-[0px_4px_20px_rgba(15,76,129,0.04)] text-white shrink-0 hover:bg-[#002645] transition-colors"
        >
      ...
      {active.length === 0 ? (
        <div className="w-full bg-white border border-[#C2C7D1] rounded-[8px] p-10 text-center text-[#42474F]">
            ...
            <div className="lg:col-span-2 relative bg-white border border-[#C2C7D1] rounded-[8px] shadow-[0px_4px_20px_rgba(15,76,129,0.04)] p-8 flex flex-col gap-6 overflow-hidden">
              ...
              <div className="flex items-start justify-between gap-4 z-10">
                <div className="flex flex-col gap-1">
                  ...
                  <h3 className="text-[18px] font-[600] leading-8 text-[#00355F]">{priority.medication_name}</h3>
                  <p className="text-[16px] font-[400] leading-6 text-[#42474F]">{priority.form ?? priority.dosage}</p>
                </div>
              ...
              <div className="border-t border-[#C2C7D1] pt-4 grid grid-cols-2 sm:grid-cols-4 gap-4 z-10">
                {[
                  ...
                  { label: "REFILLS", value: priority.refills_remaining, color: "text-[#00355F]" },
                ].map((item) => (
                  <div key={item.label} className="flex flex-col gap-1">
                    <span className="text-[12px] font-[600] tracking-[0.6px] uppercase text-[#42474F]">{item.label}</span>
                    <span className={`text-[16px] font-[700] leading-6 ${item.color ?? "text-[#0D1C2E]"}`}>{item.value}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="flex gap-4 z-10">
                <button
                  ...
                  className="px-6 py-2 bg-[#00355F] rounded-[4px] text-[14px] font-[700] text-white hover:bg-[#002645] transition-colors"
                >
                  Request Refill
                </button>
              ...
              {morningMed && (
                <div className="flex items-center gap-4 bg-white border border-[rgba(194,199,209,0.3)] rounded-[4px] p-4">
                  ...
                  <div className="flex flex-col gap-0.5">
                    ...
                    <span className="text-[16px] font-[700] leading-6 text-[#0D1C2E]">{morningMed.medication_name} {morningMed.dosage}</span>
                  </div>
                </div>
              )}
              {eveningMed && (
                <div className="flex items-center gap-4 bg-white border border-[rgba(194,199,209,0.3)] rounded-[4px] p-4">
                  ...
                  <div className="flex flex-col gap-0.5">
                    ...
                    <span className="text-[16px] font-[700] leading-6 text-[#0D1C2E]">{eveningMed.medication_name} {eveningMed.dosage}</span>
                  </div>
                </div>
              )}
              {!morningMed && !eveningMed && (
                <p className="text-[14px] text-[#42474F]">No medications scheduled today.</p>
              )}
            </div>
      ...
      {secondaryActive.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
          {secondaryActive.map((med) => (
            <div key={med.id} className="bg-white border border-[#C2C7D1] rounded-[8px] p-6 flex flex-col gap-4">
              <div className="flex items-center justify-between">
                <span className="text-[16px] font-[700] leading-6 text-[#0D1C2E]">{med.medication_name}</span>
                <span className="px-2 py-0.5 bg-[#D5E3FC] rounded-[2px] text-[10px] font-[700] text-[#42474F]">ACTIVE</span>
              </div>
              <p className="text-[14px] font-[400] leading-5 text-[#42474F]">{med.dosage} • {med.frequency}</p>
              <div className="h-px bg-[rgba(194,199,209,0.3)]" />
              <div className="flex items-center justify-between">
                <span className="text-[14px] font-[400] leading-5 text-[#42474F]">{med.expires}</span>
                <span className="text-[14px] font-[700] leading-5 text-[#0D1C2E]">{med.refills_remaining}</span>
              </div>
              <button
                onClick={() => {
                  if (med.action_label === "Request Refill") { setRefillTarget(med); setRefillSuccess(false); }
                }}
                className="w-full py-2 border border-[rgba(0,53,95,0.2)] rounded-[4px] text-[14px] font-[700] text-[#00355F] hover:bg-[#EFF4FF] transition-colors"
              >
                {med.action_label}
              </button>
            </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 text-[16px] font-[700] text-[#00355F] hover:bg-[#EFF4FF] rounded-[4px] transition-colors"
            >
              <SlidersHorizontal className="w-[18px] h-3 shrink-0" />
              Filter Records
              {filterStatus !== "All" && (
                <span className="ml-1 px-2 py-0.5 bg-[#00355F] text-white rounded-full text-[10px] font-[700]">
                  {filterStatus}
                </span>
              )}
            </button>
            {showFilterMenu && (
              <div className="absolute right-0 top-full mt-1 bg-white border border-[#C2C7D1] rounded-[8px] shadow-lg z-20 min-w-[160px] overflow-hidden">
                {(["All", "Active", "Completed", "Expired"] as FilterStatus[]).map((f) => (
                  <button
                    key={f}
                    onClick={() => { setFilterStatus(f); setShowFilterMenu(false); }}
                    className={`w-full text-left px-4 py-2.5 text-[14px] font-[600] transition-colors ${
                      filterStatus === f ? "bg-[#EFF4FF] text-[#00355F]" : "text-[#42474F] hover:bg-[#F8F9FF]"
                    }`}
                  >
                    {f}
                  </button>
          ...
          <table className="w-full min-w-[640px] border-collapse">
            <thead>
              <tr className="bg-[#DCE9FF]">
                {["Medication", "Prescribed By", "Date", "Status", "Action"].map((label, i) => (
                  <th
                    key={label}
                    className={`px-6 py-4 text-[12px] font-[600] tracking-[0.6px] uppercase text-[#42474F] ${i === 4 ? "text-right" : "text-left"}`}
                  >
                    {label}
                  </th>
                ))}
            ...
            <tbody>
              {filteredHistory.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-6 py-8 text-center text-[14px] text-[#42474F]">
                    No prescription history found.
                  </td>
                </tr>
              ) : (
                filteredHistory.map((row, idx) => (
                  <tr key={row.id} className={idx > 0 ? "border-t border-[#C2C7D1]" : ""}>
                    <td className="px-6 py-5">
                      <div className="flex flex-col gap-0.5">
                        <span className="text-[16px] font-[700] leading-6 text-[#0D1C2E]">{row.medication_name}</span>
                        <span className="text-[14px] font-[400] leading-5 text-[#42474F]">{row.form ?? row.dosage}</span>
                      </div>
                    </td>
                    <td className="px-6 py-5 pl-12 text-[14px] font-[400] text-[#0D1C2E]">{row.prescriber}</td>
                    <td className="px-6 py-5 text-[14px] font-[400] text-[#0D1C2E]">{row.prescribed_date}</td>
                    <td className="px-6 py-5">
                      <span className={`px-3 py-1 rounded-[12px] text-[12px] font-[600] tracking-[0.6px] ${
                        row.status === "Completed" ? "bg-[#DCFCE7] text-[#15803D]"
                        : row.status === "Expired" ? "bg-[#FFDAD6] text-[#93000A]"
                        : "bg-[#D5E3FC] text-[#42474F]"
                      }`}>
                        {row.status}
                      </span>
                    </td>
      ...
      {refillTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="bg-white rounded-[12px] shadow-2xl w-full max-w-[480px] flex flex-col gap-6 p-8">
            {refillSuccess ? (
              <div className="flex flex-col items-center gap-4 py-4">
                <CheckCircle2 className="w-14 h-14 text-[#15803D]" />
                <h3 className="text-[20px] font-[700] text-[#0D1C2E] text-center">Refill Request Sent</h3>
                <p className="text-[14px] text-[#42474F] text-center">
                ...
                <button
                  ...
                  className="mt-2 px-8 py-2.5 bg-[#00355F] rounded-[8px] text-white text-[14px] font-[700] hover:bg-[#002645] transition-colors"
                >
                  Done
                </button>
              </div>
              ...
              <>
                <div className="flex items-start justify-between gap-4">
                  <div className="flex flex-col gap-1">
                    <h3 className="text-[20px] font-[700] text-[#0D1C2E]">Request Refill</h3>
                    <p className="text-[14px] text-[#42474F]">{refillTarget.medication_name} — {refillTarget.dosage}</p>
                  </div>
                  <button onClick={closeRefillModal} className="p-1 rounded hover:bg-[#F8F9FF] transition-colors">
                    <X className="w-5 h-5 text-[#42474F]" />
                  </button>
                </div>

                <div className="flex flex-col gap-3 bg-[#F8F9FF] rounded-[8px] p-4">
                  {[
                    ...
                    <div key={label} className="flex justify-between text-[14px]">
                      <span className="font-[600] text-[#42474F]">{label}</span>
                      <span className="text-[#0D1C2E]">{value}</span>
                    </div>
                  ))}
                </div>

                <div className="flex flex-col gap-2">
                  <label className="text-[12px] font-[700] tracking-[0.6px] uppercase text-[#42474F]">
                  ...
                  <textarea
                    ...
                    className="w-full border border-[#C2C7D1] rounded-[8px] px-4 py-3 text-[14px] text-[#0D1C2E] resize-none focus:outline-none focus:border-[#00355F]"
                  />
                </div>
                ...
                <div className="flex gap-3">
                  <button
                    onClick={closeRefillModal}
                    className="flex-1 py-2.5 border border-[#C2C7D1] rounded-[8px] text-[14px] font-[700] text-[#42474F] hover:bg-[#F8F9FF] transition-colors"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className="flex-1 py-2.5 bg-[#00355F] rounded-[8px] text-[14px] font-[700] text-white hover:bg-[#002645] transition-colors disabled:opacity-60"
                  >
