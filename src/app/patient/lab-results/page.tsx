...
function resultIcon(name: string) {
  ...
  if (n.includes("x-ray") || n.includes("mri") || n.includes("ct") || n.includes("imaging")) {
    return <FileText className="w-[18px] h-5 text-[#00355F] opacity-40" />;
  }
  if (n.includes("urin") || n.includes("ecg") || n.includes("ekg")) {
    return <Activity className="w-[22px] h-4 text-[#00355F] opacity-40" />;
  }
  return <FlaskConical className="w-[18px] h-[18px] text-[#00355F] opacity-40" />;
}
    ...
    return (
      <div className="w-full h-screen flex items-center justify-center bg-[#F8F9FF]">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-sm font-[600] text-[#00355F]">Loading lab results...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full flex flex-col gap-12 px-6 py-6 bg-[#F8F9FF] font-[Manrope,sans-serif] text-[#42474F]">
        ...
        <div className="flex items-center gap-4 shrink-0">
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 px-5 py-2.5 border border-[#C2C7D1] rounded-[12px] text-[12px] font-[600] tracking-[0.6px] text-[#00355F] hover:bg-[#EFF4FF] transition-colors"
            >
            ...
            {showFilter && (
              <div className="absolute right-0 top-full mt-1 bg-white border border-[#C2C7D1] rounded-[8px] shadow-lg z-20 min-w-[140px] overflow-hidden">
          ...
          {results.length > 0 && (
            <DownloadPDFButton
              ...
              className="flex items-center gap-2 px-5 py-2.5 bg-[#0F4C81] rounded-[12px] text-[12px] font-[600] tracking-[0.6px] text-[#8EBDF9] hover:bg-[#0a3d6b] transition-colors"
            />
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 w-full">
        {highlights.length === 0 ? (
          <div className="sm:col-span-2 bg-[#F8F9FF] border border-[#C2C7D1] rounded-[8px] p-6 text-[14px] text-[#42474F]">
            No lab results on record yet.
          </div>
        ) : (
          highlights.map((h) => (
            <div key={h.id} className="bg-[#F8F9FF] border border-[#C2C7D1] rounded-[8px] shadow-[0px_4px_20px_rgba(15,76,129,0.04)] p-6 flex flex-col justify-between gap-8">
              <div className="flex items-start justify-between">
                <div className="flex items-center justify-center w-10 h-[50px] bg-[rgba(15,76,129,0.1)] rounded-[8px] shrink-0">
                  {resultIcon(h.name)}
                </div>
                <span className="text-[12px] font-[600] tracking-[0.6px] text-[#42474F]">{h.date}</span>
              </div>
              <div className="flex flex-col gap-1">
                <h3 className="text-[18px] font-[600] leading-8 text-[#0D1C2E]">{h.name}</h3>
                <p className="text-[14px] font-[400] leading-5 text-[#42474F]">{h.provider}</p>
              </div>
            </div>
          ))
        )}
        ...
        <div className="bg-[#00355F] rounded-[8px] shadow-[0px_4px_20px_rgba(15,76,129,0.04)] p-6 flex flex-col gap-3">
          ...
          {pendingResults.length === 0 ? (
            <>
              <h3 className="text-[18px] font-[600] leading-8 text-white">All Results Reviewed</h3>
            ...
            <>
              <h3 className="text-[18px] font-[600] leading-8 text-white">
              ...
              <div className="pt-3">
                <button
                  ...
                  className="px-4 py-2 bg-white rounded-[12px] text-[12px] font-[700] tracking-[0.6px] text-[#00355F] hover:bg-[#EFF4FF] transition-colors"
                >
                  Track Status
                </button>
              </div>
      ...
      {/* ── Results Table ── */}
      <div ref={pendingRef} className="w-full bg-white border border-[#C2C7D1] rounded-[8px] overflow-x-auto">
        {/* Table Header */}
        ...
        {/* Table Body */}
        {pageRows.length === 0 ? (
          <div className="px-6 py-10 text-center text-[14px] text-[#42474F]">
          ...
          pageRows.map((row, idx) => {
            ...
            return (
              <div key={row.id}>
                <div
                  className={`grid grid-cols-[2fr_1fr_1.5fr_1fr_auto] min-w-[640px] px-6 py-6 gap-4 items-center ${idx > 0 || isExpanded ? "border-t border-[#C2C7D1]" : ""}`}
                >
                  ...
                  <div className="flex justify-center">
                    <span className={`px-3 py-1 rounded-[12px] text-[12px] font-[600] tracking-[0.6px] ${
                      isPending ? "bg-[#DCE9FF] text-[#42474F]" : "bg-[#D4E6E5] text-[#576867]"
                    }`}>
                      {row.status}
                    </span>
                  </div>
                  <div className={`flex items-center gap-3 justify-end ${isPending ? "opacity-30" : ""}`}>
                    ...
                    <button
                      ...
                      className="p-2 rounded-[12px] hover:bg-[#EFF4FF] transition-colors"
                      title={isExpanded ? "Collapse" : "Expand details"}
                    >
                      {isExpanded
                        ? <ChevronUp className="w-3 h-[7px] text-[#42474F]" />
                ...
                {isExpanded && (
                  <div className="min-w-[640px] px-6 pb-6 border-t border-[#EFF4FF] bg-[#F8F9FF]">
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-4">
                      {[
                        ...
                        <div key={item.label} className="flex flex-col gap-1">
                          <span className="text-[11px] font-[600] tracking-[0.6px] uppercase text-[#42474F]">{item.label}</span>
                          <span className="text-[14px] font-[700] text-[#0D1C2E]">{item.value}</span>
                        </div>
                      ))}
                    </div>
                    <p className="mt-4 text-[13px] font-[400] leading-5 text-[#42474F] max-w-[600px]">
                      {isPending
        ...
        {/* Pagination Footer */}
        <div className="flex items-center justify-between px-6 py-4 bg-[#EFF4FF] border-t border-[#C2C7D1]">
          ...
          <div className="flex gap-2">
            <button
              ...
              disabled={page === 0}
              className="flex items-center justify-center w-[25px] h-9 border border-[#C2C7D1] rounded-[12px] disabled:opacity-30 hover:bg-white transition-colors"
            >
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex items-center gap-4 pt-2">
            <Link
              ...
              className="flex items-center gap-1 text-[12px] font-[700] tracking-[0.6px] text-[#00355F] hover:underline"
            >
